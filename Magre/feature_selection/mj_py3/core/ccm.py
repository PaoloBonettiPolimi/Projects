...
import mj_py3.core.kernel as kernel
        ...
        for iteration in range(1, iterations + 1):
            ...
            if verbose:
                loss = self.sess.run(self.loss, feed_dict=dict(
                    list(zip(self.inputs, inputs))))
                print("iteration {} loss {}".format(iteration, loss))

            # Update w with projected gradient method.
            gradients = self.sess.run(self.gradients, feed_dict=dict(list(zip(self.inputs, inputs))))
...
def ccm(X, Y, num_features, type_Y, epsilon, learning_rate = 0.001,
    iterations = 1000, D_approx = None, verbose = False):
    """
